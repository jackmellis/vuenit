...
module.exports = function (component, options) {
  ...
  var watchIds = [];
  [instance._watcher, child._watcher].concat(instance._watchers).concat(child._watchers).forEach(function syncWatcher(watcher){
    if (watchIds.includes(watcher.id)){
      return;
    }else{
      watchIds.push(watcher.id);
    }

    if (!watcher.lazy && !watcher.sync){
      watcher.sync = true;
    }
    if (watcher.deps && watcher.deps.length){
      watcher.deps.forEach(syncWatcher);
    }
  });
