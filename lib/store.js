...
class Store{
  ...
  dispatch(name, payload){
    var result = this._doWhen('dispatch', name, payload);
...
function createState(store, modules, state, config, path, isRoot) {
  Object.keys(config).forEach(function (key) {
    ...
    if (key === 'default' && isRoot){
      ...
      modules[mKey].context.state = localState;
...
function createGetters(store, modules, state, getters, config, path, isRoot) {
  Object.keys(config).forEach(function (key) {
    ...
    if (key === 'default' && isRoot){
      ...
      var mKey = gKey + '/';
      var params = { state : state, rootState : store.state };
      modules[mKey] = modules[mKey] || { context : { state : state, getters : {} } };
